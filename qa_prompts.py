...
import threading, traceback, tkinter as tk, qa_functions, qa_files, sys
from typing import *
...
class MessagePrompts:
    ...
    class _CInfo(threading.Thread):
        def __init__(self, root: Union[tk.Tk, tk.Toplevel], msg: InfoPacket, quit_on_close: bool):
            ...
            self.svg_size = (50, 50)
            self.svg_base = '.src\\.icons\\info_icon_{}.svg'
            self.svg_path = self.svg_base.format('dark')
            self.img = None
        ...
        def svg_set_path(self):
            bg_farther_from = qa_functions.ColorFunctions.calculate_more_contrast(
                qa_functions.HexColor('#000000'),
                qa_functions.HexColor('#ffffff'),
                self.theme.background
            ).color

            print(bg_farther_from)

            if bg_farther_from == '#ffffff': self.svg_path = self.svg_base.format('light')
            else: self.svg_path = self.svg_base.format('dark')
